...
function clickedCell(cell) {
    ...
    msg.innerHTML = "ເຈົ້າແພ້🤭";
	}
	if (emptyCells(board).length == 0 && !gameOverAll(board)) {
		var msg = document.getElementById("message");
		msg.innerHTML = "ອ໊ະ! ເຈົ້າກະເກັ່ງຢູ່ໄດ໋ນິ ແຕ່ຍັງບໍ່ເກັ່ງພໍທີ່ຈະຊະນະ";
	}
	if (gameOverAll(board) == true || emptyCells(board).length == 0) {
  
		button.value = "ເລີ່ມໃຫມ່";
		button.disabled = false;
	}
}

function restartBttn(button) {
	if (button.value == "ໃຫ້ AI ເລີ່ມ") {
		aiTurn();
		button.disabled = true;
	}
	else if (button.value == "ເລີ່ມໃຫມ່") {
		var htmlBoard;
		var msg;
